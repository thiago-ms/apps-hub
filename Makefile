...
help:
	...
	@echo "  make dist         - APK debug com a versão no nome em dist/appshub-<versão>-debug.apk"
	@echo "  make apk-release  - APK release versionado em dist/ (alias de dist-release)"
	@echo "  make dist-release - APK release (assinado + R8) em dist/appshub-<versão>-release.apk"
...
keystore: image
	$(RUN) sh -c 'mkdir -p keystore && \
	  keytool -genkeypair -v -keystore keystore/release.jks -alias appshub \
	    -keyalg RSA -keysize 2048 -validity 10000 \
	    -storepass appshub123 -keypass appshub123 \
	    -dname "CN=AppsHub, OU=Dev, O=AppsHub, L=SP, S=SP, C=BR" && \
	  printf "storeFile=keystore/release.jks\nstorePassword=appshub123\nkeyAlias=appshub\nkeyPassword=appshub123\n" > keystore/keystore.properties'
