...
tasks.register<Copy>("distApk") {
    ...
    rename { "appshub-$version-debug.apk" }
    doLast { println(">> APK versionado em: dist/appshub-$version-debug.apk") }
}
...
tasks.register<Copy>("distReleaseApk") {
    ...
    into(rootProject.layout.projectDirectory.dir("dist"))
    rename { "appshub-$version-release.apk" }
    doLast { println(">> APK versionado em: dist/appshub-$version-release.apk") }
}
